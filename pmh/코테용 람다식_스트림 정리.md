...
## Map에 List/Set을 저장할 때 자주 사용하는 패턴

### 1. put()만 사용하면 발생하는 문제

```java
Map<String, Set<String>> map = new HashMap<>();

map.put("muzi", new HashSet<>());
map.get("muzi").add("frodo");

map.put("muzi", new HashSet<>());
map.get("muzi").add("neo");
...
`put()`은 기존 값을 새로운 값으로 덮어쓰기 때문에 기존에 저장된 `frodo`가 사라진다.
...
```java
map.putIfAbsent("muzi", new HashSet<>());
map.get("muzi").add("frodo");
...
```java
map.computeIfAbsent("muzi", k -> new HashSet<>()).add("frodo");
map.computeIfAbsent("muzi", k -> new HashSet<>()).add("neo");
```
...
> "없으면 생성하고, 있으면 기존 컬렉션(List/Set)을 사용한다."

는 의미이며, 코딩테스트와 실무에서 매우 자주 사용되는 패턴이다.