...
- Python으로 1차 풀이를 시도하여 `19분`만에 문제를 해결했다.
    - 정렬 관련해 문법을 몰라서 서치를 했다…
    - 최종 제출에서 런타임 에러가 발생하여 재미니에게 물어봐 해결했다. pq가 비었을 경우를 상정하지 못한 문제였다.
- Java로 2차 풀이를 시도하여 `24분`만에 문제를 해결했다.
